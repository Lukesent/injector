...
{
	if(!m_loadLibrary) {
		auto kernel32 = GetModuleHandleA(XorStr("Kernel32.dll"));
		if(!kernel32) {
			return false;
		}

		m_loadLibrary = reinterpret_cast <uintptr_t> (GetProcAddress(kernel32, XorStr("LoadLibraryA")));
		if(!m_loadLibrary) {
			return false;
		}
	}
	...
	const auto pos = temp_name.find_last_of(XorStr("\\"));
	...
	auto Thread_handle = CreateRemoteThread(process_handle,nullptr,0, reinterpret_cast <LPTHREAD_START_ROUTINE> (m_loadLibrary), data, 0, nullptr);
